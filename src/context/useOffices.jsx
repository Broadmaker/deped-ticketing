...
export function OfficesProvider({ children }) {
  ...
  const fetchOffices = useCallback(async () => {
    ...
    try {
      ...
      const map = {}
      data.forEach(o => { map[o.id] = o.services || [] })
      setServicesMap(map)
    } catch (err) {
  ...
  async function toggleOfficeStatus(id) {
    ...
    if (!office) return
    await api.patch(`/offices/${id}`, { is_active: office.comingSoon })
    await fetchOffices()
  }
  ...
  async function addService(officeId, data) {
    const id = data.label.toLowerCase().replace(/\s+/g, '_').replace(/[^a-z0-9_]/g, '') + '_' + Date.now().toString(36).slice(-4)
